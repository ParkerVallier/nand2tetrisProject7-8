...
public class VM {
    ...
    public static void main(String[] args) throws IOException {
       ...
       // if(args.length != 0) {
    		...
    		File input = new File("BasicLoop.vm");
            String outputPath = "";					// ^^
            ...
            while (parser.hasNext()) {
	            ...
	            if (cType == Parser.ARITHMETIC) {	// Checks type, and does correct action
            	 ...
            	 else if (cType == Parser.LABEL) {

            		writer.writeLabel(parser.arg1());

            	} 
            	else if (cType == Parser.GOTO) {

            		writer.writeGo(parser.arg1());

	            }
            	else if (cType == Parser.IF) {
	
	                writer.writeIf(parser.arg1());
	
	            }
            	else if (cType == Parser.RETURN) {
	
	                writer.writeReturn();
	
	            }
            	else if (cType == Parser.FUNCTION) {
	
	                writer.writeFunction(parser.arg1(),parser.arg2());
	
	            }
            	else if (cType == Parser.CALL) {
